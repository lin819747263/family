...
const ctrl = require('../controllers/funShopController');
const { authenticate } = require('../middleware/auth');
const { verifyFamilyAccess, verifyResourceAccess } = require('../middleware/familyAccess');
const { FunShop } = require('../models');
const { idParam } = require('../middleware/validators');

router.use(authenticate);

const verifyAccess = verifyResourceAccess({
  getFamilyId: async (id) => { const item = await FunShop.findByPk(id, { attributes: ['familyId'] }); return item?.familyId; }
});
...
router.put('/:id/check', verifyAccess, idParam, ctrl.toggleCheck);
